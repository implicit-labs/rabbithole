...
import { createClaudeParser } from "./transcripts/claude.js";
import { createCodexParser } from "./transcripts/codex.js";
import { recordConfirmsRabbitholeSession } from "./confirmation.js";
import { AgentCorrelator } from "./correlate.js";
...
export class AgentContextMonitor {
  ...
  async refresh() {
    ...
    if (typeof this.correlation.validate === "function" && systemClock.now() - this.lastValidationAt >= validationInterval) {
      this.lastValidationAt = systemClock.now();
  ...
  async attach(correlation) {
    ...
    this.offset = bytes.length;
    this.reader = createNdjsonReader({
      onLine: (line) => this.parser.pushLine(line),
      onRecord: (record) => {
        if (!this.confirmed && recordConfirmsRabbitholeSession(this.correlation.agent, record, this.sessionIds())) {
          this.confirmed = true;
        }
      },
    });
  ...
  setState(next) {
    ...
    this.pending = next;
    const elapsed = systemClock.now() - this.lastEmittedAt;
  ...
  flushState() {
    ...
    this.current = next;
    this.lastEmittedAt = systemClock.now();
