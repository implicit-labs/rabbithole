...
const server = new McpServer(
  { name: "rabbithole", version: "0.1.0" },
  {
    instructions: [
      ...
      "Rabbithole is open or that you are listening unless open_rabbithole or the final answer_branch",
      "call is actually still running.",
      "",
      "Flow:",
      ...
      "For sub-agent work, the main agent remains the sole Rabbithole coordinator. After spawning a sub-agent",
      "for a request, call answer_branch with delegated=true and no content, retain its session_id/request_id,",
      "and resume listening with open_rabbithole { hole_id }. Later, the main agent completes that original",
      "request normally; delegated completions return immediately and never take over the listener. Sub-agents",
      "must not call Rabbithole. Use delegated=false if the main agent reclaims a delegated request.",
      ...
      "When the human explicitly asks you to save or send a new note to an existing canvas, use",
      "send_to_rabbithole. It is durable and never opens the browser. Do not use it speculatively,",
