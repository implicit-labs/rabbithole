...
export function comboboxMarkup(options) {
  const valueId = options.valueId || options.id + "-value";
  return (
    BUTTON_OPEN +
    ' id="' +
    escapeHtml(options.id) +
    '" class="' +
    escapeHtml(options.className || "settings-select") +
    '" type="button" aria-haspopup="listbox" aria-expanded="false" aria-labelledby="' +
    escapeHtml(options.labelledBy + " " + valueId) +
    '" data-value="' +
    ...
    escapeHtml(valueId) +
    '">' +
    escapeHtml(options.label || options.value || "") +
    "</span>" +
    (options.iconHtml || "") +
    "</button>"
  );
...
export function wireCombobox(root, options) {
  let trigger = root?.querySelector("#" + options.id),
    surface = null,
    input = null,
    list = null,
    popover = null;
  ...
  function renderResults() {
    ...
    const filtered = options.source.filter(items, query);
    let html = filtered
      .map(function (item, index) {
        return options.source.renderOption(item, {
          index: index,
          query: query,
          value: options.getValue ? options.getValue(item) : item.value,
        });
      })
      .join("");
  ...
  function renderError() {
    state = "error";
    const query = input.value.trim();
    const retry = BUTTON_OPEN + ' type="button" class="combobox-retry" data-combobox-retry>Try again</button>';
  ...
  function load() {
    ...
    Promise.resolve()
      ...
      .catch(function () {
        if (surface && token === loadToken) renderError();
      });
  }
  ...
  function commit(row) {
    if (!row) return;
    const value = row.dataset.value || row.dataset.id || "";
    const itemIndex = Number(row.dataset.itemIndex);
    const item = Number.isInteger(itemIndex) ? items[itemIndex] : null;
    const label =
      row.dataset.label ||
      row.querySelector(options.optionLabelSelector || ".model-option-name")?.textContent?.trim() ||
      value;
  ...
  function open() {
    if (surface) return;
    const listboxId = options.id + "-listbox",
    ...
    surface.innerHTML =
      ...
      (options.searchIconHtml || "") +
      '<input id="' +
      escapeHtml(inputId) +
      '" placeholder="' +
      escapeHtml(options.placeholder || "Search…") +
      '" autocomplete="off" spellcheck="false" role="combobox" aria-expanded="true" aria-controls="' +
      escapeHtml(listboxId) +
      '" aria-labelledby="' +
      escapeHtml(options.labelledBy) +
      '">' +
      (options.searchAfterHtml || "") +
      '</div><div id="' +
      escapeHtml(listboxId) +
      '" class="' +
      escapeHtml(options.listClassName || "combobox-list") +
      '" role="listbox" aria-labelledby="' +
      escapeHtml(options.labelledBy) +
      '"></div>';
    document.body.appendChild(surface);
    input = surface.querySelector("#" + inputId);
    list = surface.querySelector("#" + listboxId);
    trigger.setAttribute("aria-controls", listboxId);
    input.addEventListener("input", function () {
      state === "error" ? renderError() : state === "ready" ? renderResults() : null;
    });
    input.addEventListener("keydown", function (event) {
      ...
      if (key === "ArrowDown" || key === "ArrowUp") {
        ...
        if (rows.length) {
          event.preventDefault();
          commit(rows[Math.max(0, activeIndex)]);
        }
      }
    });
    surface.addEventListener("click", function (event) {
      if (closestEl(event.target, "[data-combobox-retry]")) {
        load();
        return;
      }
      const row = closestEl(event.target, '[role="option"]');
      if (row) commit(row);
    });
    popover = openPopover({
      trigger: trigger,
