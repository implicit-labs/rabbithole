...
import { modelDescriptor } from "./model-descriptor.js";
...
function readCache() {
  try {
    ...
    if (raw && systemClock.now() - raw.at < CACHE_TTL_MS && Array.isArray(raw.models) && raw.models.length) {
      return raw.models;
    }
  } catch {}
  return null;
...
function writeCache(models) {
  try {
    sessionStorage.setItem(CACHE_KEY, JSON.stringify({ at: systemClock.now(), models }));
  } catch {}
}
...
export function formatModelPrice(model) {
  if (!model) return "";
  const prompt = model.price?.prompt;
  const completion = model.price?.completion;
  ...
  return `$${perMillion(prompt)} · $${perMillion(completion)}`;
}
