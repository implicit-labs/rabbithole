...
/** @param {string} baseUrl @param {string} [model] @param {{signal?: AbortSignal, onProgress?: (progress: any) => void}} [options] */
...
async function queryLoopbackPermission() {
  ...
  for (const name of ["loopback-network", "local-network-access"]) {
    try {
      const permission = await navigator.permissions.query({ name: /** @type {PermissionName} */ (name) });
